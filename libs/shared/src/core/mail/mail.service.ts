...
export class MailService {
  ...
  private async getTransporter() {
    ...
    if (!config || !config.smtpEnabled) {
      try {
        ...
        this.logger.error(`Failed to create Ethereal SMTP test account: ${err.message}`);
        return null;
      }
    }
  ...
  async sendMail(to: string, subject: string, html: string, text: string) {
    ...
    try {
      ...
      if (previewUrl) {
        this.logger.log(`[Ethereal SMTP] Email Preview URL: ${previewUrl}`);
        console.log(`\n📧 [Ethereal SMTP] Email sent to ${to}`);
