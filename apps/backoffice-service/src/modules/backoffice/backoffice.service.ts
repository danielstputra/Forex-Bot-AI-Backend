...
export class BackofficeService {
  ...
  async getTenantBranding(tenantId: string | undefined) {
    // Seed a mock tenant if none exists to facilitate testing
    const tenantCount = await this.prisma.tenant.count();
    if (tenantCount === 0) {
      await this.prisma.$transaction(async (tx) => {
        const newTenant = await tx.tenant.create({
          data: {
            name: 'Apex Capital White-Label',
            domain: 'tenant-a.com',
            logoUrl: 'https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?auto=format&fit=crop&w=120&h=120&q=80',
            status: 'ACTIVE'
          }
        ...
        await tx.tenantTheme.create({
          data: {
            ...
            primaryColor: '#e11d48', // Vibrant Rose
            secondaryColor: '#4f46e5', // Indigo
            fontFamily: 'Outfit',
            customCss: '.sidebar { background: #0f172a; }'
          }
        });
      });
    }
    ...
    if (tenantId) {
      ...
      if (tenant) {
        return {
          ...
          theme: {
            primaryColor: tenant.theme?.primaryColor || '#06b6d4',
            secondaryColor: tenant.theme?.secondaryColor || '#7c3aed',
