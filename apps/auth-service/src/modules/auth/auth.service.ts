...
export class AuthService implements OnModuleInit {
  ...
  async onModuleInit() {
    console.log('[AuthService] Checking for default admin account...');
    const adminEmail = 'admin@forexbot.ai';
    const existing = await this.prisma.user.findUnique({ where: { email: adminEmail } });
    if (!existing) {
      console.log('[AuthService] Seeding default admin account (admin@forexbot.ai)...');
      const passwordHash = await hashPassword('Password123!');
      try {
        await this.prisma.$transaction(async (tx) => {
          let dbRole = await tx.role.findUnique({ where: { name: 'SUPERADMIN' } });
          if (!dbRole) {
            dbRole = await tx.role.create({
              data: { name: 'SUPERADMIN', description: 'SUPERADMIN system role' }
            });
          }
        ...
        console.log('[AuthService] Default admin account seeded successfully.');
      } catch (err: any) {
        console.error('[AuthService] Failed to seed default admin account:', err.message);
      }
    }
  }
  ...
  private async enforceSessionLimit(tx: any, userId: string, tier: string) {
    ...
    if (activeSessions.length >= maxSessions) {
      ...
      for (const session of sessionsToRevoke) {
        ...
        // Emit a Redis Pub/Sub event for real-time ForceLogout
        this.redis.publish(
  ...
  async verifyAccount(token: string, ipAddress: string, userAgent: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      await this.enforceSessionLimit(tx, updatedUser.id, tier);
  ...
  async login(body: any, ipAddress: string, userAgent: string) {
    ...
    // Ambil konfigurasi aplikasi untuk memeriksa apakah OTP diaktifkan (Hanya diaktifkan di luar development/testing)
    const appConfig = await this.prisma.appConfig.findFirst();
    const isDev = process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test';
    if (appConfig?.loginOtpEnabled && !isDev) {
      ...
      // Print OTP directly to console for quick and easy local testing
      console.log(`\n==================================================`);
      console.log(`🔑 [OTP SECURITY] LOGIN OTP FOR ${user.email}: ${otpCode}`);
      console.log(`==================================================\n`);
